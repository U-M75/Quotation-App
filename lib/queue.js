...
function queuePayloadFromUpdate(update) {
  ...
  try {
    return JSON.parse(body.slice(markerIndex + QUEUE_MARKER.length).trim());
  } catch {
    return null;
  }
}
...
export async function queueMondayUpdate(update) {
  ...
  console.log(
    ...
    `processing after ${WAIT_MS / 1000} seconds`
  );
}
...
export async function getDueMondayUpdates() {
  ...
  const data = await mondayRequest(`
    query {
      updates(limit: 100, from_date: "${fromDate}", to_date: "${toDate}") {
        id
        body
        item_id
        created_at
  ...
  for (const update of data.updates || []) {
    const pending = queuePayloadFromUpdate(update);
    ...
    if (Number(pending.notBefore) <= Date.now()) {
      due.push({
        ...
        itemId: String(pending.itemId || update.item_id),
      });
    }
  }
...
export async function removeMondayUpdate(queueUpdateId) {
  ...
  console.log(`Removed Monday queue update ${queueUpdateId}`);
}

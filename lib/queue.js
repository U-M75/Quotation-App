...
export async function queueMondayUpdate(update) {
  ...
  await redis.sadd(
    PENDING_SET,
    String(update.itemId)
  );

  console.log(
    `Queued Monday update for ${update.itemId}. ` +
    `It will be processed after ${WAIT_MS / 1000} seconds.`
  );
}
...
export async function removeMondayUpdate(itemId) {
  ...
  console.log(
    `Removed Monday update for ${itemId} from Redis queue`
  );
}

...
export default function ProposalForm() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/quotation/proposal/${encodeURIComponent(token)}`)
      .then(async response => {
        ...
        if (!response.ok || !data.success) throw new Error(data.error || 'Unable to load proposal');
  ...
  const submitProposal = async event => {
    ...
    try {
      ...
      const response = await fetch(`/api/quotation/proposal/${encodeURIComponent(token)}`, {
        method: 'POST',
        body: data,
      });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.error || 'Unable to submit proposal');
      setSuccess(true);
    } catch (submitError) {
      ...
      <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fbdce6 0%, #c7eaf9 100%)', color: colors.brown, padding: '26px 16px 48px' }}>
        <div style={{ maxWidth: '760px', margin: '0 auto' }}>
          <header style={{ textAlign: 'center', marginBottom: '24px' }}>
            <img className="ksc-logo" src="/logo.png" alt="Kawaii Slime Company" style={{ display: 'block', width: '140px', height: '140px', objectFit: 'contain', margin: '0 auto 8px' }} />
            <h1 className="ksc-title" style={{ margin: 0, color: colors.brown, fontSize: '32px' }}>KSC Proposal Form</h1>
          ...
          {!loading && error && !project && (
            <section style={{ background: colors.white, borderRadius: '14px', padding: '30px', textAlign: 'center', color: colors.pink }}>
              {error}
            </section>
          )}
            ...
            <>
              ...
              <section className="ksc-card" style={{ background: colors.white, border: `1px solid ${colors.border}`, borderRadius: '14px', padding: '26px', boxShadow: '0 10px 30px rgba(139, 94, 59, 0.12)' }}>
                ...
                <form onSubmit={submitProposal}>
                  ...
                  {pdf && <p style={{ margin: '-8px 0 18px', fontSize: '13px', color: colors.blueDeep }}>Selected: {pdf.name}</p>}
